...
| `run_pipeline_v3.py` | v3 管线：ETF行业回测 | 按需 |
| `run_pipeline_v4.py` | **v4 管线：个股回测 + 质量门动量** | 按需 |
...
| 模块 | 说明 |
|------|------|
| `engine.py` | 周频回测：无前视偏差、信号时序修正、A股真实成本（印花税+佣金+滑点）、现金约束、先卖后买、close列名自适应 |
| `evaluation.py` | 绩效评估：Sharpe/回撤/IC-IR/归因报告 |

**v4 个股回测** (`run_pipeline_v4.py`)：37-100只AI个股、PEG+PRG混合信号、质量门动量(sigmoid)、6年回测：年化47%、Sharpe 1.06、IC 0.012。证明了个股层面因子远优于ETF。
...
2. **IC 只是参考，不是真理**：IC 高可能是过拟合，IC 低可能是时间尺度失配。季度级因子预测周频收益，IC 天然偏低。不要用 IC 驱动因子调整
3. **修正序列是校准源**：当分析师共识上修时信号应为正，下修时信号应为负——这个方向正确性比 IC 数值重要
4. **PEG 的"G"质量比 PEG 数值重要**：38% 的 AI 结构性增速 vs 38% 的化工供给侧反弹——PEG 相同，含义完全不同
5. **动量需质量门**：A股动量诱人但危险。基本面质量门 (sigmoid) 让好公司涨时跟、弱公司涨时不跟
6. **允许犯错，追踪犯错**：记录每次因子信号与后续实际走势的偏差，但不立即调整因子。偏差积累到形成模式时再调整
